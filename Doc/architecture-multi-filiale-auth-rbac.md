...
> - 🟡 Phase 3 — authentification. **Fait** : cookies httpOnly (access 15 min + refresh 30 jours avec rotation et détection de réutilisation), `POST /auth/refresh`, logout qui révoque réellement (avant : no-op), CSRF double-submit cookie, `helmet` actif, frontend migré (plus de token en `localStorage`, `withCredentials`, intercepteur refresh-retry). **Pas fait** : 2FA (TOTP), rate limiting dédié sur les routes d'auth, table d'audit. Vérifié en HTTP réel (curl + cookie jar) côté backend ; **pas de test navigateur réel** (aucun outil de rendu disponible) — à valider manuellement avant de construire le 2FA par-dessus.
> - ⬜ Phase 4 — RBAC finalisé. Le bypass `SUPER_ADMIN` dans `RoleGuard`/`checkRole` est déjà fait (avancé depuis la phase 2, nécessaire pour promouvoir un compte sans casser son accès au reste de l'app). Restent : suppression des champs legacy `userRole`/`additionalRoles`, garde de route par rôle généralisée côté frontend, et un audit systématique des controllers `@SetMetadata('roles', ...)` — 5 en avaient déjà un sans `RoleGuard` réellement branché (trouvé par accident en phase 2, corrigé, mais pas de balayage exhaustif fait).
...
- Nouveau segment de routes `/super-admin/*` (TanStack Router), distinct de `/dashboard/*`, gardé par rôle `SUPER_ADMIN` (voir chantier 3.3 pour la garde réelle).
- `/dashboard/*` reste la vue par filiale, utilisée par tous les autres rôles.
- Un composant "sélecteur de filiale" partagé (drill-down : le super-admin peut filtrer la vue consolidée sur une filiale précise, sans jamais pouvoir élargir le scope d'un non-super-admin) — même logique que `resolveSubsidiaryFilter` côté gmo.
- `Sidebar.tsx` : ajouter le cas `SUPER_ADMIN` au switch existant (le mécanisme de menu par rôle est déjà en place, juste à étendre).
