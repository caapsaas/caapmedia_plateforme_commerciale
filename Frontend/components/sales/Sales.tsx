...
const Sales: React.FC = () => {
    ...
    const [appliedFilters, setAppliedFilters] = useState<FindAllOrdersDto>(initialFilterState);

    const { data: filteredOrders = [], isLoading: isLoadingOrders } = useQuery<Order[]>({
    ...
    if (isLoadingOrders || isLoadingProducts || !contacts) {
                            ...
                            <div className="p-6 pt-0">
                                <div className="p-4 bg-slate-50 rounded-lg -mx-6 px-6 -mt-0 pt-6">
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                        <SelectFilter name="customerId" label={t('filter.client')} value={filters.customerId || ''} onChange={e => setFilters(prev => ({ ...prev, customerId: e.target.value }))} options={clientOptions} placeholder={t('filter.allClients')} />
                                        <SelectFilter name="productId" label={t('filter.product')} value={filters.productId || ''} onChange={e => setFilters(prev => ({ ...prev, productId: e.target.value }))} options={productOptions} placeholder={t('filter.allProducts')} />
                                        <SelectFilter name="orderStatus" label={t('filter.orderStatus')} value={filters.orderStatus || ''} onChange={e => setFilters(prev => ({ ...prev, orderStatus: e.target.value as OrderStatus }))} options={orderStatusOptions} placeholder={t('filter.allOrderStatuses')} />
