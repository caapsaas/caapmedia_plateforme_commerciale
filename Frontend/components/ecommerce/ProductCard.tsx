...
const ProductCard: React.FC<ProductCardProps> = ({ product, isLiked, onLike, onUnlike, onViewDetails }) => {
    const { t } = useI18n();
    ...
    const handleOrderViaWhatsApp = () => {
        const message = [
            ...
            hasImages ? `🖼️ Photo : ${activeImageUrl}` : null,
            '',
            'Pouvez-vous me communiquer le prix et les délais ? Merci !',
        ].filter(Boolean).join('\n');
            ...
            <div className="p-4 flex flex-col flex-grow justify-between">
                ...
                <div className="flex items-center gap-2 mt-4">
                    ...
                    <button
                        ...
                        className="flex-1 min-w-0 px-2 py-1.5 font-semibold rounded-lg text-xs transition-all flex items-center justify-center gap-1 bg-[#25D366] text-white hover:bg-[#128C7E]"
                    >
                        <svg className="w-3.5 h-3.5 shrink-0" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <span className="truncate">{t('ecommerce.orderViaWhatsApp')}</span>
                    </button>
                    <button
                        onClick={() => onViewDetails(product)}
                        className="flex-1 min-w-0 px-2 py-1.5 font-semibold rounded-lg text-xs transition-colors flex items-center justify-center gap-1 border border-slate-300 text-slate-700 hover:bg-slate-100"
                    >
                        <IconSearch className="w-3.5 h-3.5 shrink-0" />
                        <span className="truncate">{t('ecommerce.viewDetails')}</span>
                    </button>
                </div>
            </div>
