...
export class TaxesService {
  ...
  async update(id: string, updateTaxDto: UpdateTaxDto) {
    ...
    if (updateTaxDto.isDefault === false) {
      ...
      if (currentTax?.isDefault) {
        throw new BadRequestException('Impossible de désactiver la taxe par défaut. Veuillez définir une autre taxe comme étant par défaut pour la remplacer.');
      }
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      return tx.taxRate.update({
        ...
        data: {
          ...updateTaxDto,
          ...(updateTaxDto.rate && { rate: new Prisma.Decimal(updateTaxDto.rate) }),
        },
      });
    });
  }
  ...
  async remove(id: string) {
    ...
    if (tax.isDefault) {
      throw new BadRequestException('Impossible de supprimer la taxe par défaut.');
    }
