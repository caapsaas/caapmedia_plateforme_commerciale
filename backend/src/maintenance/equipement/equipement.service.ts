...
export class EquipementService {
    ...
    async findOne(id: string) {
        const equipment = await this.prisma.equipment.findUnique({ where: { id }, include: this.includeAll });
        if (!equipment) {
    ...
    async update(id: string, updateEquipementDto: UpdateEquipmentDto, user: any) {
        ...
        const equipment = await this.prisma.equipment.update({ where: { id }, data: {
            ...updateEquipementDto,
            subsidiaryId: user.subsidiaryId,
    ...
    async remove(id: string) {
        await this.findOne(id);
        const equipment = await this.prisma.equipment.delete({ where: { id }, include: this.includeAll });
        return equipment;
    }
    ...
    if (query.equipmentName) {
        where.equipmentName = { contains: query.equipmentName, mode: 'insensitive' };
    }
    ...
    if (query.acquisitionFromDate || query.acquisitionToDate) {
        where.acquisitionDate = {
            gte: query.acquisitionFromDate ? new Date(query.acquisitionFromDate) : undefined,
            lte: query.acquisitionToDate ? new Date(query.acquisitionToDate) : undefined,
        };
    }
    ...
    if (query.lastMaintenanceFromDate || query.lastMaintenanceToDate) {
        where.lastMaintenanceDate = {
            gte: query.lastMaintenanceFromDate ? new Date(query.lastMaintenanceFromDate) : undefined,
            lte: query.lastMaintenanceToDate ? new Date(query.lastMaintenanceToDate) : undefined,
        };
    }
    ...
    if (query.nextMaintenanceFromDate || query.nextMaintenanceToDate) {
        where.nextMaintenanceDate = {
            gte: query.nextMaintenanceFromDate ? new Date(query.nextMaintenanceFromDate) : undefined,
            lte: query.nextMaintenanceToDate ? new Date(query.nextMaintenanceToDate) : undefined,
        };
    }
